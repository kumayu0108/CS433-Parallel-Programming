...
__global__ void area_kernel (float a, float b, unsigned long long num_intervals_per_thread)
{
	...
	__shared__ float local_area2[THREADS_PER_BLOCK/32];
	...
	for (i=warpSize/2; i>0; i=i/2) {
		local_area[threadIdx.x] += local_area[((threadIdx.x + i) % warpSize) + (threadIdx.x/warpSize)*warpSize];
		__syncwarp(0xffffffff); // No need to synchronize all threads, but this is low-cost
	}
	if ((threadIdx.x % warpSize) == 0) local_area2[threadIdx.x/warpSize] = local_area[threadIdx.x];
	__syncthreads();
	if ((threadIdx.x/(THREADS_PER_BLOCK/32)) == 0) {
		for (i=THREADS_PER_BLOCK/64; i>0; i=i/2) {
			local_area2[threadIdx.x] += local_area2[(threadIdx.x + i) % (THREADS_PER_BLOCK/32)];
			__syncwarp(0xffffffff);	// No need to synchronize all threads, but this is low-cost
		}
		if (threadIdx.x == 0) atomicAdd(&area, 0.5*local_area2[threadIdx.x]);
	}
}	
