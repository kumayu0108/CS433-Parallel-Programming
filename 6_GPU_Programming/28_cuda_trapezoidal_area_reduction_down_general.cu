...
__global__ void area_kernel (float a, float b, unsigned long long num_intervals_per_thread)
{
	...
	for (i=warpSize/2; i>0; i=i/2) local_area += __shfl_down_sync(mask, local_area, i);
	if ((threadIdx.x % warpSize) == 0) local_area2[threadIdx.x/warpSize] = local_area;
	__syncthreads();
	if ((threadIdx.x/(THREADS_PER_BLOCK/32)) == 0) {
		local_area = local_area2[threadIdx.x];
		for (i=THREADS_PER_BLOCK/64; i>0; i=i/2) local_area += __shfl_down_sync(mask, local_area, i);
		if (threadIdx.x == 0) atomicAdd(&area, 0.5*local_area);
	}
}	
