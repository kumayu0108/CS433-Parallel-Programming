...
int main (int argc, char *argv[])
{
	...
	cudaMallocManaged((void **)&private_sum, nthreads*sizeof(double));
	...
	if (nthreads < 16) {
		reduction_kernel<<<1, nthreads>>>(a, SIZE/nthreads, private_sum);
	}
	else {
		reduction_kernel<<<nthreads/16, 16>>>(a, SIZE/nthreads, private_sum);
	}
