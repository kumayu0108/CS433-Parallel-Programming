...
{
   int pid, P, done, n, iters = 0, nthreads;
   ...
   if (!pid) {
      sprintf(buffer, "het_iterative_equation_solver_outfile_%d_%d_%d.txt", n, P, nthreads);
      fp = fopen(buffer, "w");
   ...
   while (!done) {
      local_diff = 0.0;
#pragma omp parallel for num_threads (nthreads) reduction(+:local_diff)
      for (int i=0; i<n/P; i++) {
	 next_x[i] = b[i];
         for (int j=0; j<n; j++) {
