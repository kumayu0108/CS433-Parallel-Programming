...
int insert_parallel (int v)
{
	list_node_t *prev=NULL, *curr;
        list_node_t *node=NULL;
	if (head) pthread_rwlock_wrlock(&head->lock);
	curr = head;
        while (curr && (curr->v < v)) {
		if (prev != NULL) pthread_rwlock_unlock(&prev->lock);
		prev = curr;
                curr = curr->next;
		if (curr) pthread_rwlock_wrlock(&curr->lock);
        }
        if (curr && (curr->v == v)) {
...
{
	list_node_t *prev=NULL, *curr;
	if (head) pthread_rwlock_rdlock(&head->lock);
	curr = head;
        while (curr && (curr->v < v)) {
                prev = curr;
                curr = curr->next;
